    ...
    timesactScriptNew = async function ($) {
        ...
        class Selectors {
            ...
            setNormalSelectors() {
                ...
                this.buttonSelector = $(this.ids.addToCartButton)
                ...
                if (this.ids.message) {
                    this.messageSelector = $(this.ids.message)
                }
        ...
        class ScriptRunner {
            ...
            isActiveOnPreorder(variants) {
                return Object.values(variants).every((variant) => variant.config.status === "ACTIVE" || variant.config.status === "NO_STOCK")
            }
            ...
            enableButtonOnCollection(item, shop, product, defaultSettings, productId) {
                ...
                for (let variantId in product.variants) {
                    ...
                    if (configType === "config") {
                        ...
                        const settingTemplate = window.ta.settings[product.variants[variantId].config.settingsTemplate]
                        let cartValue = settingTemplate ? settingTemplate.cart.labelText.value : defaultSettings.cart.labelText.value
                        let cartKey = settingTemplate ? settingTemplate.cart.labelText.key : defaultSettings.cart.labelText.key
                        const isCartEnabled = settingTemplate ? settingTemplate.cart.labelText.isEnabled : defaultSettings.cart.labelText.isEnabled
